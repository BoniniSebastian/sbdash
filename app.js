/* SB Dash v4.1 — FIX:
   - Pomodoro syntax fix + stable ring (no NaN)
   - Under-nav clicks work (no JS crash)
   - Weather loads (no JS crash)
   - Wheel: allow scrolling inside view pages; change view only at edges
*/
(() => {
  "use strict";
  ...
  try {
    ...
    // ---------- Wheel behavior (IMPORTANT: do not kill scroll) ----------
    const mainPanel = document.querySelector(".mainPanel");

    function getActivePageEl() {
      if (!track) return null;
      // current viewPage is the one at currentIndex
    ...
    if (mainPanel) {
      mainPanel.addEventListener(
        "wheel",
        (e) => {
          // Let the active viewPage scroll if it can scroll in that direction
          ...
          if (canScroll) {
            ...
            // If at an edge, we can switch view instead
          }
    ...
    function setStroke(pct) {
      ...
      // Use positive dashoffset to "drain"
    ...
    function loop() {
      ...
      // Use floor for stable countdown
    ...
    console.error("SB Dash crashed:", err);
    // Optional: you can show something in UI later if you want
  }
})();
