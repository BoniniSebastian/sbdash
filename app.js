/* SB Dash v4.2 — Stable iPad/desktop
   - Views + dial with live icon + snap
   - Wheel: scroll inside view pages, switch view only at edges
   - Todo/Ideas/Done/Superprio with swipe
   - News RSS via proxies + pull-to-refresh
   - Weather via Open-Meteo
   - Timer (Pomodoro) with SVG ring (Safari + Chrome safe) + top header timer bar (only when running)
*/
(() => {
  "use strict";
...
if (calHandle) {
  ...
  const ICONS = {
    ...
    pomodoro: "assets/ui/icon-pomodoro.svg", // rename file if you like, but keep path or update here
  };
  ...
  // ---------- Wheel behavior (do not kill scroll) ----------
  const mainPanel = document.querySelector(".mainPanel");

  function getActivePageEl() {
    if (!track) return null;
    const pages = track.querySelectorAll(".viewPage");
    ...
    mainPanel.addEventListener(
      ...
      (e) => {
        ...
        if (canScroll) {
          ...
          // If user scrolls within bounds, allow default scroll
          if ((delta < 0 && !atTop) || (delta > 0 && !atBottom)) {
            return; // allow normal scrolling
          }
        }

        // Switch view when not scrolling content (or at edges)
        e.preventDefault();
        if (wheelCooldown) return;
        wheelCooldown = true;

        setViewByIndex(currentIndex + (delta > 0 ? 1 : -1));
 ...
 if (dialEl) {
  ...
  // ---------- Timer (Pomodoro) + header timer bar ----------
  ...
  // Timer bar in Main header
  const timerBar = $("timerBar");
  const timerBarTime = $("timerBarTime");
  ...
  function setColor(pct) {
    if (!pomoProg) return;
    const p = Math.max(0, Math.min(1, pct));
    if (p > 0.40) pomoProg.style.stroke = "rgba(0,209,255,.88)";
    else if (p > 0.15) pomoProg.style.stroke = "rgba(255,165,0,.88)";
  ...
  function renderPomo() {
    ...
    if (pomoSub) {
      if (!running && safeLeft === total) pomoSub.textContent = "Redo";
      else if (running && safeLeft > 0) pomoSub.textContent = "Fokus…";
      else if (!running && safeLeft > 0) pomoSub.textContent = "Pausad";
      else pomoSub.textContent = "KLAR";
    }

    // --- Update top timer bar (only when running) ---
    if (timerBar && timerBarFill && timerBarTime) {
      if (running && safeLeft > 0) {
        timerBar.classList.add("show");
        timerBar.setAttribute("aria-hidden", "false");

        timerBarTime.textContent = `${pad2(mm)}:${pad2(ss)}`;

        const p = Math.max(0, Math.min(1, pct));
        timerBarFill.style.transform = `scaleX(${p})`;

        // match ring color thresholds
        if (p > 0.40) timerBarFill.style.background = "rgba(0,209,255,.85)";
        else if (p > 0.15) timerBarFill.style.background = "rgba(255,165,0,.88)";
        else timerBarFill.style.background = "rgba(255,70,70,.88)";
      } else {
        hideTimerBar();
      }
    }
  ...
  pomoBtns.forEach((btn) => btn.addEventListener("click", () => setMinutes(btn.dataset.pomo)));
